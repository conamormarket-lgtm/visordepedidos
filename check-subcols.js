...
async function run() {
    try {
        const subCols = ["variaciones", "opciones", "prendas", "stock", "items", "colores", "tallas", "variantes"];
        for (const colName of subCols) {
            console.log(`Checking sub-collection 'inventarios/inventario-prendas/${colName}'...`);
            const colRef = collection(db, "inventarios", "inventario-prendas", colName);
